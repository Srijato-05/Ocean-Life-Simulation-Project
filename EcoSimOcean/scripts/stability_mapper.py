...
def set_param(sim_config, fauna_configs, param_key, value):
    key = param_key.replace('_prey', '').replace('_predator', '').replace('_scav', '')
    if key in sim_config:
        sim_config[key] = value
    elif param_key.endswith('_prey') or key in fauna_configs.get("Zooplankton", {}):
        fauna_configs["Zooplankton"][key] = value
    elif param_key.endswith('_predator') or key in fauna_configs.get("SmallFish", {}):
        fauna_configs["SmallFish"][key] = value
    elif param_key.endswith('_scav') or key in fauna_configs.get("Crab", {}):
        fauna_configs["Crab"][key] = value
    else:
        print(f"Warning: Parameter '{param_key}' not found in any config.")

def run_stability_map(map_config):
    print(f"\n--- Starting Analysis for: {map_config['title']} ---")
    ...
    for i, y_val in enumerate(y_range):
        for j, x_val in enumerate(x_range):
            ...
            if history:
                results["prey_pop"][i, j] = history[-1].get('zooplankton', 0)
                results["pred_pop"][i, j] = history[-1].get('small_fish', 0)
                results["scav_pop"][i, j] = history[-1].get('crab', 0)
    return results

def create_dominance_grid(prey_grid, pred_grid, scav_grid):
    dominance_grid = np.zeros_like(prey_grid)
    collapse_mask = (prey_grid < 10) | (pred_grid < 10) | (scav_grid < 10)
...
# --- HIGHLIGHT: Plotting is now split into two distinct 4-panel dashboard functions ---
...
def plot_core_dashboard(results, map_config):
    ...
    # 1. Fitness Score Heatmap
    im = axes[0, 0].imshow(results['fitness'], cmap='viridis', origin='lower', aspect='auto', norm=plt.matplotlib.colors.LogNorm())
    fig.colorbar(im, ax=axes[0, 0], label='Fitness Score (Log Scale)')
    format_heatmap_axes(axes[0, 0], 'Overall Fitness Score')

    # 2. Ecosystem Outcome Heatmap
    dominance_grid = create_dominance_grid(results['prey_pop'], results['pred_pop'], results['scav_pop'])
    ...
    fig.colorbar(im, ax=axes[1, 0], label='Final Prey Population')
    format_heatmap_axes(axes[1, 0], 'Prey (Zooplankton) Population')

    # 4. Predator Population Heatmap
    im = axes[1, 1].imshow(results['pred_pop'], cmap='Reds', origin='lower', aspect='auto')
    fig.colorbar(im, ax=axes[1, 1], label='Final Predator Population')
    format_heatmap_axes(axes[1, 1], 'Predator (SmallFish) Population')
...
def plot_analytics_dashboard(results, map_config):
    ...
    mean_scav = np.mean(results['scav_pop'], axis=0)
    p1, = ax.plot(x_range, mean_prey, 'b-', label='Avg Prey')
    p2, = ax2.plot(x_range, mean_pred, 'r-', label='Avg Pred')
    p3, = ax2.plot(x_range, mean_scav, 'g--', label='Avg Scav')
    ax.set_ylabel('Avg Prey Population', color='b')
    ax2.set_ylabel('Avg Pred/Scav Population', color='r')
    ...
    ax = axes[0, 1]
    sizes = np.clip(results['pred_pop'].flatten(), 5, 500)
    scatter = ax.scatter(X.flatten(), Y.flatten(), c=results['fitness'].flatten(), s=sizes, cmap='viridis', norm=plt.matplotlib.colors.LogNorm(), alpha=0.7)
    fig.colorbar(scatter, ax=ax, label='Fitness Score')
    ax.set_title('Fitness (Color) & Pred Pop (Size)')
    ax.set_xlabel(map_config["x_param"])
    ax.set_ylabel(map_config["y_param"])
    ...
    if len(counts) > 0:
        ax.pie(counts, labels=pie_labels, autopct='%1.1f%%', startangle=90, colors=pie_colors)
