...
This version can tune parameters for prey, predators, and scavengers.
"""
...
class Particle:
    ...
    def _initialize_params(self):
        ...
        for key, bounds in self.param_bounds.items():
            ...
            # --- HIGHLIGHT: Added logic for Scavenger parameters ---
            elif key.endswith('_scav'):
                self.fauna_config["Crab"][sim_key] = random.uniform(min_val, max_val)
            # Fallback for keys that aren't species-specific
            elif sim_key in self.fauna_config.get("Zooplankton", {}):
                self.fauna_config["Zooplankton"][sim_key] = random.uniform(min_val, max_val)
            elif sim_key in self.fauna_config.get("SmallFish", {}):
                self.fauna_config["SmallFish"][sim_key] = random.uniform(min_val, max_val)
            elif sim_key in self.fauna_config.get("Crab", {}):
                self.fauna_config["Crab"][sim_key] = random.uniform(min_val, max_val)
    ...
    def update_velocity(self, global_best_sim, global_best_fauna, pso_config):
        ...
        for key in self.velocity.keys():
            ...
            # --- HIGHLIGHT: Added logic for Scavenger parameters ---
                ...
                p_best_val = self.best_fauna_config["Crab"][sim_key]
                g_best_val = global_best_fauna.get("Crab", {}).get(sim_key, p_best_val)
                current_val = self.fauna_config["Crab"][sim_key]
            # Fallback logic
            elif sim_key in self.fauna_config.get("Crab", {}):
                 p_best_val = self.best_fauna_config["Crab"][sim_key]
                 g_best_val = global_best_fauna.get("Crab", {}).get(sim_key, p_best_val)
    ...
    def update_position(self):
        ...
        for key, vel in self.velocity.items():
            ...
            # --- HIGHLIGHT: Added logic for Scavenger parameters ---
                ...
                self.fauna_config["Crab"][sim_key] += vel
                self.fauna_config["Crab"][sim_key] = max(min_val, min(self.fauna_config["Crab"][sim_key], max_val))
            # Fallback logic
            elif sim_key in self.fauna_config.get("Crab", {}):
                self.fauna_config["Crab"][sim_key] += vel
                self.fauna_config["Crab"][sim_key] = max(min_val, min(self.fauna_config["Crab"][sim_key], max_val))
            ...
            if any(k in key for k in ["period", "count", "threshold", "age", "lifespan"]):
                if key.endswith('_prey'):
                    ...
                    self.fauna_config["Crab"][sim_key] = int(self.fauna_config["Crab"][sim_key])